...
var con= require('./../dbconnection').default;
...
module.exports.teachercurrentresult = async function(req,res){
  ...
  console.log(users.testid)
  con.query("SELECT COUNT(sno) as total,AVG(marks) as avg,Max(marks) as max,Min(marks) as min from result where testId=(select testid from test where temail='"+users.email+"' ORDER BY Date LIMIT 0, 1)", function(err , data){
    if (err) {
      console.log(err);
      return res.json({data:null});
    } else {
...
module.exports.studentcurrentresult = async function(req,res){
  ...
  con.query("SELECT COUNT(sno) as total,AVG(marks) as avg,Max(marks) as max,Min(marks) as min FROM `result` WHERE `semail`='"+users.email+"'", function(err , data){
    if (err) {
      console.log(err);
      return res.json({data:null});
    } else {
